 ...
 * Layout must match the HDL nvme_s1/nvme_cnfg_slave region map.
 * ============================================================ */
...
long vfpga_nvme_init(struct vfpga_dev *device, struct nvme_init_ioctl *req) {
    ...
    if (ds) {
        ...
        /* P2P doorbell mapping: map the SSD's BAR0 (where its doorbell registers live) into the
         * FPGA's DMA domain, so the FPGA's outbound doorbell write (FPGA->SSD MMIO) carries a bus
         * address valid under the FPGA-side IOMMU. This is the FPGA->SSD mirror of the iova_base
         * mapping above (SSD->FPGA), and the same principle as the GPU P2P path (vfpga_gup.c), which
         * maps a peer's resource into the FPGA's (bd->pci_dev) bus address space. Without an IOMMU
         * dma_map_resource returns the raw PA, so the doorbell works either way. */
