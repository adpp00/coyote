/**
 * axi_direct_2ch_split
 *
 * Splits 1 AXI4 slave into 2 AXI4 masters using address-bit interleaving.
 * addr[SPLIT_BIT] == 0 → port 0, addr[SPLIT_BIT] == 1 → port 1.
 * Address is passed through UNCHANGED (no bit stripping).
 *
 * W channel follows AW routing via a small FIFO.
 * R and B responses use round-robin arbitration for fairness.
 */
