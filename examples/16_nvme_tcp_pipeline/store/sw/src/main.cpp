/**
 * Example 16 — Pipelined Store: TCP → NVMe (FPGA-side host)
 *
 * This program:
 *   1. Initializes NVMe device 0
 *   2. Allocates HBM ring buffer (default 128MB)
 *   3. Configures FPGA pipeline registers
...
/* Register map (must match nvme_tcp_pipe_store_ctrl HW) */
enum Reg : uint32_t {
    CTRL        = 0,
    STATUS      = 1,
    LISTEN_PORT = 2,
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== NVMe TCP Pipelined Store ===" << std::endl;
