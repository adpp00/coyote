 ...
 *  11 (RO)  : BYTES_SENT      - Total TCP bytes sent
 *  12 (RO)  : WR_PTR          - NVMe write pointer (slot index)
 *  13 (RO)  : RD_PTR          - TX read pointer (slot index)
 */
...
localparam integer REG_BYTES_SENT  = 11;
localparam integer REG_WR_PTR      = 12;
localparam integer REG_RD_PTR      = 13;
    ...
    if (aresetn == 1'b0)
        ...
        if (ctrl_reg_wren) begin
            case (axi_awaddr[ADDR_LSB+:ADDR_MSB])
                REG_CTRL, REG_LISTEN_PORT, REG_HBM_BASE,
                REG_CHUNK_BITS, REG_SLOT_MASK, REG_NSID: begin
...
always_ff @(posedge aclk) begin
    if (aresetn == 1'b0) begin
        ...
        if (~axi_awready && axi_ctrl.awvalid && axi_ctrl.wvalid && aw_en) begin
            axi_awready <= 1'b1; aw_en <= 1'b0; axi_awaddr <= axi_ctrl.awaddr;
...
always_ff @(posedge aclk) begin
    if (aresetn == 1'b0) begin axi_arready <= 1'b0; axi_araddr <= '0; end
    else if (~axi_arready && axi_ctrl.arvalid) begin axi_arready <= 1'b1; axi_araddr <= axi_ctrl.araddr; end
    else axi_arready <= 1'b0;
end

always_ff @(posedge aclk) begin
    if (aresetn == 1'b0) begin axi_rvalid <= 1'b0; axi_rresp <= 2'b0; end
    else if (axi_arready && axi_ctrl.arvalid && ~axi_rvalid) begin axi_rvalid <= 1'b1; axi_rresp <= 2'b0; end
    else if (axi_rvalid && axi_ctrl.rready) axi_rvalid <= 1'b0;
end
