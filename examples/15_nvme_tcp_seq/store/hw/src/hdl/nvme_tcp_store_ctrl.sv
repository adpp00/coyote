/**
 * nvme_tcp_store_ctrl — AXI-Lite control register parser
 *
 ...
 *  10 (RO)  : BYTES_RECV      - Total TCP bytes received
 *  11 (RO)  : NVME_LBA_OFF    - Current NVMe LBA byte offset
 */

import lynxTypes::*;

module nvme_tcp_store_ctrl (
    input  logic                        aclk,
    input  logic                        aresetn,
...
localparam integer REG_BYTES_RECV  = 10;
...
always_ff @(posedge aclk) begin
    if (aresetn == 1'b0) begin
        ...
        case (axi_araddr[ADDR_LSB+:ADDR_MSB])
            ...
            REG_BYTES_RECV:  axi_rdata         <= bytes_recv;
