/**
 * NVMe-TCP Read Client — counterpart for Example 16 Read
 *
 * Protocol:
 *   1. TCP connect to FPGA
 *   2. Send 64B meta: [7:0] = total_read_length (little-endian)
 *   3. Receive total_read_length bytes (FPGA sends in 32KB TCP packets)
 *
 * Build:
 *   g++ -O2 -o read_client read_client.cpp
 *
 * Usage:
 *   ./read_client -i 10.0.0.1 -p 5002 -s 128M
 *   ./read_client -i 10.0.0.1 -p 5002 -s 1G -o /tmp/nvme_dump.bin
 */
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== NVMe-TCP Read Client ===" << std::endl;
    ...
    std::cout << "Timeout: " << timeout_sec << " s" << std::endl;

    // --- 1. Connect ---
    int fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd < 0) { perror("socket"); return 1; }

    // Enlarge socket recv buffer
    int rcvbuf = 4 * 1024 * 1024;
    setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &rcvbuf, sizeof(rcvbuf));

    // Disable Nagle so the 64B meta is sent immediately
    int nodelay = 1;
    ...
    memcpy(meta + 8, &read_len, sizeof(read_len));  // [127:64]
    ...
    // For verify: keep first 64 bytes
    ...
    while (total_recv < read_len) {
        ...
        if (n <= 0) {
            if (n == 0)
                std::cerr << "\nConnection closed by FPGA (received "
                          << total_recv << " / " << read_len << ")" << std::endl;
            else
                perror("\nrecv");
            break;
        }

        // Save first bytes for verify
        if (verify && !got_first) {
    ...
    if (total_recv == read_len)
        std::cout << "Status:   OK (all bytes received)" << std::endl;
    else
        std::cout << "Status:   INCOMPLETE (" << total_recv << " / " << read_len << ")" << std::endl;
